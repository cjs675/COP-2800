...
| when creating a static method in a class in a class & instantiate 100 objects, only one copy exists in memory & the method doesn't receive a __this__ reference                    | when creating a nonstatic method in a class & instantiating 100 objects, only one copy of the method exists in memory, but the method receives a __this__ reference that contains the address of the object currently using it |
